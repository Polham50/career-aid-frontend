...
export var AppPhase;
(function (AppPhase) {
    AppPhase[AppPhase["INIT"] = 0] = "INIT";
    AppPhase[AppPhase["HOME"] = 1] = "HOME";
    AppPhase[AppPhase["SIGNUP"] = 2] = "SIGNUP";
    AppPhase[AppPhase["LOGIN"] = 3] = "LOGIN";
    AppPhase[AppPhase["DASHBOARD"] = 4] = "DASHBOARD";
    AppPhase[AppPhase["SUBSCRIPTION"] = 5] = "SUBSCRIPTION";
    AppPhase[AppPhase["MOCK_INTERVIEW"] = 6] = "MOCK_INTERVIEW";
    AppPhase[AppPhase["AWAITING_CONFIRMATION"] = 7] = "AWAITING_CONFIRMATION";
    AppPhase[AppPhase["FORGOT_PASSWORD"] = 8] = "FORGOT_PASSWORD";
    AppPhase[AppPhase["RESET_PASSWORD"] = 9] = "RESET_PASSWORD";
})(AppPhase || (AppPhase = {}));
...
const App = () => {
    ...
    const submitAssessment = useCallback(async (answers, childId) => {
        if (!user?._id && !childId) {
    ...
    const renderContent = () => {
        switch (state.phase) {
            case AppPhase.HOME: return _jsx(HomePage, { onStart: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.SIGNUP }) });
            case AppPhase.SIGNUP: return _jsx(SignUpPage, { onSignUpSuccess: (email, token) => {
                    dispatch({ type: 'SET_USER_EMAIL_CONFIRMATION', email });
                    dispatch({ type: 'SET_ACTION_TOKEN', token });
                    dispatch({ type: 'SET_PHASE', phase: AppPhase.AWAITING_CONFIRMATION });
                }, onGoToLogin: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.LOGIN }) });
            case AppPhase.LOGIN: return _jsx(LoginPage, { onLoginSuccess: handleLoginSuccess, onGoToSignUp: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.SIGNUP }), onGoToForgotPassword: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.FORGOT_PASSWORD }), notification: state.notification });
            case AppPhase.AWAITING_CONFIRMATION:
                if (!state.userEmailForConfirmation && !state.actionToken) {
                    dispatch({ type: 'RESET' });
                    return null;
                }
                return _jsx(AwaitingConfirmationPage, { email: state.userEmailForConfirmation, token: state.actionToken, onConfirmSuccess: () => {
            ...
            case AppPhase.FORGOT_PASSWORD: return _jsx(ForgotPasswordPage, { onGoToLogin: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.LOGIN }) });
            case AppPhase.RESET_PASSWORD:
                if (!state.actionToken) {
                    dispatch({ type: 'SET_PHASE', phase: AppPhase.LOGIN });
                    return null;
                }
                return _jsx(ResetPasswordPage, { token: state.actionToken, onResetSuccess: () => {
            ...
            case AppPhase.DASHBOARD:
                if (!user) {
                    dispatch({ type: 'RESET' });
                    return _jsx(HomePage, { onStart: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.SIGNUP }) });
                }
                return _jsx(DashboardPage, { user: user, careerProfiles: state.careerProfiles, onSubmitAssessment: submitAssessment, error: state.error, onUpgrade: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.SUBSCRIPTION }), onStartInterview: (career) => dispatch({ type: 'SET_SELECTED_CAREER', career }), isSubscribed: isSubscribed, redirectToRecommendations: state.redirectToRecommendations, onRedirectConsumed: () => dispatch({ type: 'SET_REDIRECT', redirect: false }), onErrorAcknowledged: () => dispatch({ type: 'SET_ERROR', error: null }) });
            case AppPhase.SUBSCRIPTION: return _jsx(SubscriptionPage, { user: user, onBack: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.DASHBOARD }), onSubscriptionSuccess: handleSubscriptionSuccess });
            case AppPhase.MOCK_INTERVIEW:
                if (!state.selectedCareer) {
                    dispatch({ type: 'SET_PHASE', phase: AppPhase.DASHBOARD });
                    return null;
                }
                return _jsx(MockInterviewPage, { careerName: state.selectedCareer, onEndInterview: () => dispatch({ type: 'SET_SELECTED_CAREER', career: null }) });
            default: return _jsx(HomePage, { onStart: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.SIGNUP }) });
        }
    };
    ...
    return (_jsx(AppContext.Provider, { value: { user, setUser, isSubscribed, setIsSubscribed }, children: _jsxs("div", { className: "min-h-screen bg-gray-100 text-gray-800 flex flex-col", children: [(state.isLoading || state.phase === AppPhase.INIT) && (_jsx("div", { className: "fixed inset-0 bg-gray-900 bg-opacity-70 flex items-center justify-center z-50", children: _jsx(Spinner, { message: state.isLoading ? state.loadingMessage : 'Initializing...' }) })), _jsx(Header, { user: user, onLogoClick: () => dispatch({ type: 'RESET' }), onSignUp: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.SIGNUP }), onLogin: () => dispatch({ type: 'SET_PHASE', phase: AppPhase.LOGIN }), onLogout: handleLogout }), _jsx("main", { className: mainContainerClasses, children: state.phase !== AppPhase.INIT && renderContent() }), _jsx(Footer, {})] }) }));
};
export default App;
